...
> items (security tooling, branded Line/Column, testing depth, watch mode, LSP,
> plugin system) left unstruck and tracked in `TODO_LIST.md` / `ROADMAP.md`.
...
| -- | --------------------------------------------------------------------------- | ------ | ------ | ------------ |
~~| 1  | Add `govulncheck` + `gosec` to flake.nix devShell                           | HIGH   | 10 min | Security     |~~ still open — tracked in `TODO_LIST.md`
~~| 2  | Brand `ValidationError.Line/Column` as typed ints                           | HIGH   | 20 min | Type model   |~~ still open — tracked in `TODO_LIST.md`
~~| 3  | Propagate actual Column in `finding.go` instead of hardcoded `1`            | MED    | 5 min  | Correctness  |~~ still open — tracked in `TODO_LIST.md`
| 4  | Add snapshot tests for JSON/YAML/SARIF output                               | HIGH   | 30 min | Testing      |
~~| 5  | Write `CONTRIBUTING.md` with dev setup instructions                         | MED    | 15 min | Docs         |~~ done at `b72a1be`
...
~~| 8  | Switch error wrapping to `go-faster/errors`                                 | MED    | 45 min | Architecture |~~ Won't implement — stdlib `errors` + `fmt.Errorf` kept deliberately
...
~~| 12 | Write FEATURES.md (honest feature inventory)                                | MED    | 20 min | Docs         |~~ done at `50487d3`
~~| 13 | Write TODO_LIST.md (short-term actionable tasks)                            | MED    | 15 min | Docs         |~~ done at `50487d3`
...
~~| 17 | Add `--fail-on-warning` flag for non-error findings                         | LOW    | 30 min | Feature      |~~ Won't implement — single severity (syntax errors) by design
~~| 18 | Improve GitHub Action (`action.yml`) with inputs for all flags              | MED    | 30 min | CI           |~~ done at `acfe5c4`, `fba9fe5` (`path` + `args` inputs)
~~| 19 | Add `--install-hook` command for pre-commit setup                           | LOW    | 30 min | Feature      |~~ Won't implement — `.pre-commit-hooks.yaml` covers ecosystem discovery
...
~~| 22 | Add `go-fix` and `modernize` to BuildFlow linting                           | LOW    | 10 min | Tooling      |~~ done at `ed0607f` (modernizations applied in-code)
...
~~| 25 | Write ROADMAP.md (long-term direction)                                      | LOW    | 20 min | Docs         |~~ done at `50487d3`
